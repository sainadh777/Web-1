...
function handleSearch(response) {
    ...
    if (response.length > 0) {
        tableBody.append($('<tr><th></th><th>SKU</th><th>Vendor</th><th>Category</th><th>Manufacturer\'s ID</th><th>Retail</th></tr>'));
...
function populateEditForm(response) {
    ...
    document.getElementById("editImage").setAttribute("src", "/~jadrn052/public_html/proj1/my_pics" + columns[0].toLowerCase() + ".jpg");
...
function handleDuplicateSKU(response) {
    if (response != 0) {
        ...
        document.getElementById("submitbutton").disabled = true;
        document.getElementById("skustatus").innerHTML = "Provided SKU already exists.";
...
function handleDuplicateEditSKU(response) {
    if (response != 0) {
        ...
        document.getElementById("editsubmitbutton").disabled = true;
        document.getElementById("editskustatus").innerHTML = "Provided SKU already exists.";
...
function validateRetail() {
    ...
    if (!pattern.test(retail)) {
        alert("Retail must be a valid amount. (ex: $1.50 or $.50)");
        return false;
    }
    return true;
